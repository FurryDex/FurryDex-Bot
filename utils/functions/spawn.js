...
async function isXMinutesPassed(message, client) {
  try {
    ...
    if (message.content === "!spawn" && members.has(message.author.id))
      bypass = true;
        ...
        `https://api.imgur.com/3/album/${card.album}`,
        {
          headers: {
            Authorization: `Bearer ${
              require("../../config.json").imgur.accesToken
            }`,
          },
        }
      );
        ...
        new ButtonBuilder()
          ...
          .setLabel(
            locales.button.text[serverConfig.locale] ??
              locales.button.text.default
          )
          .setStyle(ButtonStyle.Danger)
      );
      let title =
        locales.embed.title[serverConfig.locale] ?? locales.embed.title.default;
      const embed = new EmbedBuilder()
        .setTitle(title)
        .setColor(
          require("../colors.json").find((color) => (color.name = "RED")).hex
        )
        .setImage(card.image);
      if (is_event)
              ...
              return new ActionRowBuilder().addComponents(
                row.components.map((button) => {
                  return new ButtonBuilder()
                    ...
                    .setDisabled(true); // Toggle the disabled state
                })
              );
            });
            msg
              .edit({ embeds: msg.embeds, components: newComponents })
              .catch(() => {});
          }, 300_000);
        });
