...
async function anticheat_update(client) {
	...
	users.forEach(async (user) => {
		...
		if (messages.length <= 3) return; // Pas assez de messages pour calculer le pourcentage
		let pourcent = (messages.map((x) => (x.have_spawn_card + (x.userCard == x.user_id ? 1 : 0)) * 1.5)?.reduce((a, b) => a + b, 0) / messages.length) * 100;
